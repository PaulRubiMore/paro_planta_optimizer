# =============================================================================
# OPTIMIZADOR AVANZADO DE PARADA DE PLANTA - REFUERZO AUTOMÁTICO DE TÉCNICOS
# =============================================================================
...
# CONFIGURACIÓN APP
# -------------------------------------------------------------------------
st.set_page_config(page_title="Optimización Parada Planta Avanzada", page_icon="🏭", layout="wide")
st.title("🏭 Optimización de Parada de Planta - Avanzada")
...
# -------------------------------------------------------------------------
# OPTIMIZADOR AVANZADO CON REFUERZO ESCALABLE
# -------------------------------------------------------------------------
def optimizar_asignacion_avanzada(df_actividades, horas_paro):
    ...
    # Definir número dinámico de técnicos según carga de trabajo por especialidad y centro
    tecnicos = {}
    for c in centros:
        tecnicos[c] = {}
        for e in especialidades:
            # Calcular total horas de actividades de esa especialidad
            total_horas = df_actividades[(df_actividades['centro']==c) & (df_actividades['especialidad']==e)]['duracion_h'].sum()
            n_tecnicos = ceil(total_horas/capacidad_tecnico)
            n_tecnicos = max(n_tecnicos,1)  # mínimo 1 técnico
            tecnicos[c][e] = [f"{c}_{e}_T{i+1}" for i in range(n_tecnicos)]
    ...
    # Cada actividad debe tener cobertura total
    for idx, row in df_actividades.iterrows():
        act_id = f"{row['orden']}_{row['especialidad']}"
        centro = row['centro']
        esp = row['especialidad']
        model.Add(sum([variables[(act_id,t)]['assigned']*variables[(act_id,t)]['dur'] for t in tecnicos[centro][esp]]) 
                  >= row['duracion_h'])
        ...
        for e in especialidades:
            ...
            for t in techs:
                acts = [variables[(f"{r['orden']}_{r['especialidad']}",t)] 
                        for _,r in df_actividades[(df_actividades['centro']==c)&(df_actividades['especialidad']==e)].iterrows()]
    ...
    # Restricción: capacidad máxima por técnico
    for c in centros:
        for e in especialidades:
    ...
    # Objetivo: minimizar hora final y priorizar criticidad
    criticidad_peso = {'ALTA':1000,'MEDIA':100,'BAJA':10}
    makespan_vars = []
    for (act_id,t), v in variables.items():
        orden_num = int(act_id.split('_')[0])
        crit = df_actividades[df_actividades['orden']==orden_num]['criticidad'].values[0]
...
if archivo1 and archivo2:
    ...
    df_cronograma = optimizar_asignacion_avanzada(df_actividades, horas_paro)
    st.subheader("Cronograma optimizado con refuerzo escalable")
